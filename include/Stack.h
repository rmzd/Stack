...
public:
    ...
    T FindMin() const;  // найти минимальный элемент
...
inline TStack<T>::TStack(int len_) : TStack() {
    ...
    if (len_ > 0) {
        len = len_;
        data = new T * [len]();  // инициализация нулями
    }
}
...
inline TStack<T>::TStack(const TStack& obj) : TStack() {
    if (obj.len > 0) {
        ...
        for (int i = 0; i < obj.top; i++) {
            if (obj.data[i]) data[i] = new T(*obj.data[i]); // копирование элементов
        }
    }
    top = obj.top;
}
...
// конструктор с внешним массивом
template<class T>
...
inline void TStack<T>::Resize(int len_) {
    ...
    if (len_ < len) {  // удаляем лишние элементы
